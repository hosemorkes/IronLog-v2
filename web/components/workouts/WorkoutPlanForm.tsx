...
function parseWeightKg(v: string | number | null | undefined): number {
...
/**
 * Преобразовать ответ GET /user/plans/:id в строки конструктора.
 */
export function planDetailToBuilderExercises(
  plan: WorkoutPlanDetail,
): BuilderExercise[] {
  return [...plan.exercises]
    .sort((a, b) => a.order - b.order)
    .map((row: PlanExerciseRow) => {
      ...
      const setsCount = Math.max(1, row.sets);
      const w = parseWeightKg(row.weight_kg);
      const reps = Math.max(1, Math.round(Number(row.reps)));
      const sets: BuilderSetRow[] = Array.from({ length: setsCount }, () => ({
        weightKg: w,
        reps,
      }));
      const rest =
 ...
 * Число строк таблицы → sets; вес и повторы из первого сета (ограничение бэкенда).
 */
...
export interface WorkoutPlanFormProps {
  defaultName: string;
  defaultItems: BuilderExercise[];
  backHref: string;
  pageTitle: string;
  submitLabel: string;
  submitPendingLabel: string;
  isSubmitting: boolean;
  onSubmit: (payload: CreateWorkoutPlanPayload) => Promise<void>;
}

/**
 * Общий конструктор плана: название, упражнения, сеты, отдых, итоги.
 */
export function WorkoutPlanForm({
  ...
  async function handleSave() {
    ...
    try {
      await onSubmit(buildPlanPayload(trimmed, items));
    } catch (e) {
      setFormError(
        e instanceof Error ? e.message : "Не удалось сохранить план.",
      );
    }
  }
    ...
    <div className="flex min-h-full flex-1 flex-col bg-bg-dark">
      <header className="flex shrink-0 items-center justify-between border-b border-[#232323] px-5 py-3.5">
        ...
        <button
          ...
          className="shrink-0 rounded-full bg-accent px-4 py-1.5 text-[13px] font-semibold text-white disabled:opacity-50"
        >
                ...
                {item.sets.map((setRow, setIdx) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      value={
                        setRow.weightKg === 0 ? "" : String(setRow.weightKg)
                      }
                      onChange={(e) =>
...
function RestRow({ value, onChange, label }: RestRowProps) {
  const options = useMemo(() => {
    const set = new Set<number>([...REST_PRESETS, value]);
