...
| `/exercises/[id]` | `web/app/(app)/exercises/[id]/page.tsx` | Карточка упражнения; фон шапки — **`getMediaUrl(image_url)`**. Блоки «Статистика» и «Личный рекорд» — заглушки (`—`) до API прогресса. В «Технике выполнения» — шаги из **`technique_steps_ru`**, fallback на **`technique_steps`** (EN); 1–2 кадра (`image_url`, `image_url_2`): статичная картинка или CSS-анимация (1.5 с); **моб.** ширина 100% / 200px, **md+** `max-w-sm` по центру / 280px; keyframes в **`globals.css`**. |
