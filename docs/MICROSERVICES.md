...
**Текущий репозиторий:** в корневом **`docker-compose.yml`** по умолчанию **закомментированы** сервисы **`worker`** (Dramatiq) и **`admin`** (Next.js на :3002); в файле есть комментарий `# TODO: раскомментировать …`. Очереди RabbitMQ и API по-прежнему работают, но фоновые задачи не исполняются, пока не запущен воркер (контейнер или локальный `dramatiq`). Для продакшена URL фронта при сборке **web** используйте **`docker-compose.override.yml.example`** → `docker-compose.override.yml` (см. **«Деплой на VPS»** в `README.md`).
...
**В compose:** сервис может быть отключён (закомментирован); тогда описанное ниже выполняется только после раскомментирования и `docker compose up`.
...
**В compose:** сервис может быть отключён (закомментирован); каталог **`admin/`** в репозитории сохраняется для локальной разработки и будущего включения.
